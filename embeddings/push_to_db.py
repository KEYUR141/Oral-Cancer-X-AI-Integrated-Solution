...
def load_papers(filepath):
    try:
        ...
        logger.error(f"Error loading papers from {filepath}: {e}")
        raise
...
def push_to_db(papers):
    try:
        ...
        logger.info(f"On-topic papers after filter: {len(final_papers)}")
        ...
        logger.info(f"Inserted {inserted} papers in total.")
    except Exception as e:
...
def main():
    ...
    logger.info(f"Loaded {len(papers)} papers. Pushing to database...")
    push_to_db(papers)
